#ifndef STRIP_PACKING_FLOW_HPP
#define STRIP_PACKING_FLOW_HPP
...
namespace strip_packing::exact::flow {
...
static inline solution_t solve(GRBEnv env, const instance_t& instance) {
    ...
    for (size_t j = 0; j < N; j++) {
        ...
        model.addConstr(expr <=
                            instance.recipient_length * vars.item_level[j][j],
                        constr_name);
    }
    ...
    for (size_t i = 0; i < N; i++) {
        max_weight += instance.rects[i].weight;
    }
