...
// left trim
//static inline string &ltrim(std::string &str) {
...
// convert a long long int value to string
inline std::string LongLongIntToString(long long int val)
{ std::stringstream out; out << val; return out.str(); }
//{return to_string(val);}  
...
class StringTable {
public:
  StringTable(int nrows,istream &file);
  ...
  // first returns in value the first element of the corresponding columnname
  bool first(string columnname,int &value); 
  bool first(string columnname,string &value); 
  bool first(string columnname,double &value); 
