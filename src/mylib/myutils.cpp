...
// 
StringTable::StringTable(int nr,istream &file) {
  string word,h;
  this->nrows = nr;
...
bool StringTable::first(string columnname, string &value){
  int col=this->columnindex(columnname);
  if (col == -1) { cout << "ERROR: Table does not contain column name \"" <<
      columnname << "\"." << endl; return(false); } 
  value = line[0][col];
  return(true);
}
bool StringTable::first(string columnname, int &value){
  int col=this->columnindex(columnname);
  if (col == -1) { cout << "ERROR: Table does not contain column name \"" <<
      columnname << "\"." << endl; return(false); } 
  value = StringToInt(line[0][col]);
  return(true);
}
bool StringTable::first(string columnname, double &value){
  int col=this->columnindex(columnname);
  if (col == -1) { cout << "ERROR: Table does not contain column name \"" <<
      columnname << "\"." << endl; return(false); } 
  value = StringToDouble(line[0][col]);
  return(true);
}
